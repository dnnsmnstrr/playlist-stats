...
export function MultiPlaylistStats() {
  ...
  const [playlistStats, setPlaylistStats] = useState<
    { name: string; id: String; analysis: ArtistAnalysis }[]
  >([]);
  ...
  useEffect(() => {
    ...
    const stats = keys.map((key) => {
      ...
      ) as ArtistAnalysis & { name: string };
      const playlistName = key.replace("playlist_", "");
      return {
        name: cachedAnalysis.name || playlistName,
    ...
    stats.forEach(({ analysis: cachedAnalysis }) => {
      Object.entries(cachedAnalysis.gender).forEach(([key, value]) => {
        aggregateAnalysis.gender[key] =
          (aggregateAnalysis.gender[key] || 0) + value;
      });
      Object.entries(cachedAnalysis.languages).forEach(([key, value]) => {
        aggregateAnalysis.languages[key] =
          (aggregateAnalysis.languages[key] || 0) + value;
      });
      Object.entries(cachedAnalysis.releaseYears).forEach(([key, value]) => {
        aggregateAnalysis.releaseYears[key] =
          (aggregateAnalysis.releaseYears[key] || 0) + value;
      });
      Object.entries(cachedAnalysis.ages).forEach(([key, value]) => {
        aggregateAnalysis.ages[key] =
          (aggregateAnalysis.ages[key] || 0) + value;
      });
      Object.entries(cachedAnalysis.countries).forEach(([key, value]) => {
        aggregateAnalysis.countries[key] =
          (aggregateAnalysis.countries[key] || 0) + value;
      });
    });

    // Sort stats by the latest release year
    stats.sort((a, b) => {
      const latestYearA = Math.max(
        ...Object.keys(a.analysis.releaseYears).map(Number)
      );
      const latestYearB = Math.max(
        ...Object.keys(b.analysis.releaseYears).map(Number)
      );
        ...
        <tbody>
          {playlistStats.map(({ name, analysis, id }) => {
            ...
            const averageAge = Math.round(
              Object.entries(analysis.ages).reduce(
                (sum, [age, count]) => sum + Number(age) * count,
                0
              ) /
                Object.values(analysis.ages).reduce(
                  (sum, count) => sum + count,
                  0
                )
            );
            const malePercentage = calculatePercentages(
              analysis.gender,
              "male"
            );
            const femalePercentage = calculatePercentages(
              analysis.gender,
              "female"
            );
            const germanPercentage = calculatePercentages(
              analysis.languages,
              "German"
            );
            const englishPercentage = calculatePercentages(
              analysis.languages,
              "English"
            );
            return (
